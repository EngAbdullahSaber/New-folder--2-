...
import { replaceDetailsUnlessExcluded } from '@/shared'
...
const MenuItem: ForwardRefRenderFunction<HTMLLIElement, MenuItemProps> = (props, ref) => {
  ...
  useEffect(() => {
    let href = rest.href || (component && typeof component !== 'string' && component.props.href)

    let path = pathname

    href = typeof href === 'string' ? href.replace('//', '/') : href

    path = replaceDetailsUnlessExcluded(pathname)
    href = replaceDetailsUnlessExcluded(href as string)

    if (href) {
      if (exactMatch ? path === href : activeUrl && path.includes(activeUrl)) {
        setActive(true)
      } else {
        setActive(false)
      }
