...
const NominationList = ({ scope }: ComponentGeneralProps) => {
  ...
  const columns = useMemo<DynamicColumnDef[]>(
    () => [
      { accessorKey: 'id', header: 'id', width: '5%', fetchKey: 'nomination_number' },
      // {
  ...
  return (
    <ListComponent
      ...
      columns={columns}
      apiEndpoint={scope == 'hr' ? '/ses/requests-status' : '/ses/requests-status-department'}
      routerUrl={`/ses/nomination/${scope}`}
      collapsible={true}
      enableFilters={true}
      enableColumnVisibility={true}
      maxVisibleColumns={8}
      fetchKey='nomination_number'
      deleteApiEndpoint='/ses/nominations'
      extraQueryParams={{ nomination_number: 'not_null' }}
    />
  )
}
