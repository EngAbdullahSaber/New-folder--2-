...
import { Box, DynamicColumnDef, Tooltip } from '@/shared'
import { getDictionary } from '@/utils/getDictionary'
import { useAppSettings } from '../../seasonal-job-request/details/hooks'

const DocumentVerificationApproval = ({ scope }: ComponentGeneralProps) => {
  ...
  const [appSetting, setAppSetting] = Shared.useState<any>(null)

  // Stable dummy formMethods to prevent re-fetching in useAppSettings
  const dummyFormMethods = Shared.useMemo(() => ({ setValue: () => {} }), [])
  ...
  const columns = Shared.useMemo<DynamicColumnDef[]>(
    () => [
...

      {
        ...
        enableFilter: true,
        type: 'selectIcon',
        list: Shared.genderOptions,
        filterType: 'select',
        width: '5%',
        filterOptions: Shared.genderOptions,
        ...
        cell: ({ row }: any) => {
          return row?.original?.elected_days && row?.original?.elected_daily_price ? (
            <Tooltip title={row.original.elected_days} placement='top'>
              <span>
                {Shared.formatNumber(row.original.elected_days * row.original.elected_daily_price, 0, {
                  locale: 'en-SA',
                  useCurrency: false
                })}
              </span>
  ...
  return (
    <Shared.ListComponent
      ...
      columns={columns}
      apiEndpoint={scope === 'department' ? '/ses/requests-status-department' : '/ses/requests-status'}
      routerUrl={`/apps/ses/document-verification-approval/${scope}`}
      collapsible={true}
      enableFilters={true}
      enableColumnVisibility={true}
      maxVisibleColumns={8}
      fetchKey={scope === 'department' ? 'doc_verification_approval_number' : 'hr_approval_number'}
      showOnlyOptions={['edit', 'delete', 'search', 'print', 'export']}
      extraQueryParams={
        scope === 'department'
          ? { doc_verification_approval_status: 'null', emp_nomination_approval_number: 'not_null' }
          : { hr_approval_status: 'null', doc_verification_approval_status: 'not_null' }
      }
    />
  )
}
export default DocumentVerificationApproval
