...
const RequestStatusDetails = ({ scope }: ComponentGeneralProps) => {
  ...
  type pathFormData = Shared.InferFieldType<typeof staticFields>
  ...
  const {
    ...
    routerUrl: { default: `/apps/ses/request-status/${scope}` },
    fields: [...staticFields, ...statusFields, ...desireFields]
  })
  ...
  Shared.useEffect(() => {
    ...
    if (mode === 'show' && dataModel && elected_daily_price && elected_days) {
      formMethods.setValue('elected_bonus', elected_daily_price * elected_days)
    }
    if (mode === 'show' && dataModel && appointed_daily_price && appointed_days_no) {
      formMethods.setValue('appointed_bonus', appointed_daily_price * appointed_days_no)
    }
  }, [mode, dataModel])
