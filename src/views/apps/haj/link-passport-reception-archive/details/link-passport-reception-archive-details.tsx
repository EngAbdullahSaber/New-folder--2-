...
const LinkPassportReceptionArchiveDetailsPage = ({ scope }: ComponentGeneralProps) => {
  ...
  const fields: Shared.DynamicFormFieldProps[] = [
...

    {
      ...
      label: 'fetch_type',
      type: 'select',
      options: Shared.passportArchiveFetchList,
  ...
  const {
    ...
    apiEndpoint: '/haj/passport/archiving',
    routerUrl: { default: `/apps/haj/link-passport-reception-archive/${scope}` },
    ...
    onSaveSuccess: async (response, mode) => {
      if (mode === 'add')
        navigateWithQuery(`/apps/haj/link-passport-reception-archive/${scope}/list`, router, locale as Shared.Locale)
    },
    initialDetailsData: {
      hajjees: []
    },
    ...
    classifiedObjects: [
      {
        ...
        fields: [...basicInformationFields, ...hajjRepresentativeSpcFields].filter(
          f => f.name !== 'card_status_id' && f.name !== 'reception_no'
        )
      },
      {
        objectName: 'card',
    ...
    customDetailValidators: {
      hajjees: (row: any, rowIndex: number) => {
        ...
        const hasShelf = !!row.shelf_no
        const hasCabinet = !!row.cabinet_no

        // Skip rows that have neither shelf nor cabinet — they will be excluded from the payload
        if (!hasShelf && !hasCabinet) {
          return { valid: true, errors: [] }
        }

        // Cross-validate: has shelf but no cabinet
        if (hasShelf && !hasCabinet) {
          errors.push({
        ...
        // Cross-validate: has cabinet but no shelf
        if (hasCabinet && !hasShelf) {
          errors.push({
            fieldName: 'shelf_no',
  ...
  const handleGetInfo = Shared.useCallback(async () => {
    ...
    // return
    if (!fetchType) {
      setWarning('دلالة جلب البيانات مطلوبة')
      return
    }
    ...
    try {
      const { data } = await Shared.apiClient.post(
        `/haj/recepted-hajjees/data`,
        { all: { reception_no: receptionId }, per_page: 1000 },
        {
          headers: {
      ...
      if (
        ...
        const updated = info.map((item: any) => ({
          nation_id: item.applicantData?.ad_current_nationality_id,
          passport_no: item.applicantData?.ad_passport_no,
          passport_unit_id: passportUnit,
          department_id: ownerDepartment,
          // applicant_data_id: item.applicant?.ad_application_no,
          applicant_data_id: item.applicantData?.ad_full_name_ar ?? item.applicantData?.ad_full_name_en,
          ad_gender: item.applicantData?.ad_gender,
          ad_date_of_birth: item.applicantData?.ad_date_of_birth,
          reception_no: receptionId
        }))
        updateDetailsData('hajjees', updated)
      }
  ...
  }, [getValues, accessToken, currentLocale, setValue, setDetailsData, ownerDepartment, passportUnit, accessToken])
    ...
    try {
      ...
      if (data.data.length === 0) {
        ...
        if (receptionId) {
          ...
          if (!result?.receptedHajjes || !result?.receptedHajjes?.reception_no) {
            setDetailError('hajjees', rowIndex, 'passport_no', "هذا الحجي غير موجود بالخدمة")
            return
          }
          else {
            updateDetailsData(
              'hajjees',
              {
                ...
                ad_date_of_birth: result['ad_date_of_birth'],
                ad_gender: result['ad_gender'],
                reception_no: result.receptedHajjes.reception_no
              },
              rowIndex
            )
          }
...
export default LinkPassportReceptionArchiveDetailsPage
