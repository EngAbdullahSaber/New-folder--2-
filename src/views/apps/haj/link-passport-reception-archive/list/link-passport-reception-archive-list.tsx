...
const LinkPassportReceptionArchiveList = ({ scope }: ComponentGeneralProps) => {
  ...
  const columns: DynamicColumnDef[] = [
    ...
    // },
    {
      accessorKey: 'passportArchiving.serial_no',
      header: 'serial',
      width: '5%',
      showPrimary: false,
      align: 'text-center',
  ...
  const handleRowOptionClick = useCallback(
    async (action: string, id: any, row: any) => {
      if (action === 'delete') {
        const applicant_data_id = row?.applicant_data_id
  ...
  return (
    <ListComponent
      ...
      routerUrl={`/apps/haj/link-passport-reception-archive/${scope}`}
      showOnlyOptions={['add', 'search', 'print', 'export']}
...
export default LinkPassportReceptionArchiveList
