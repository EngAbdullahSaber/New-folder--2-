...
export const useAppSettings = (
  ...
  Shared.useEffect(() => {
    const fetchAppSettings = async () => {
      ...
      try {
        const { data } = await Shared.apiClient.get('/haj/app-settings-by-season', {
          headers: {
    ...
    const shouldFetch = mode === 'add' || mode === 'search' || mode === 'show' || mode === 'edit' || mode === 'list'

    if (shouldFetch) {
      fetchAppSettings()
    }
  }, [accessToken, mode, dataModelId, locale, shouldBlockAccess])
